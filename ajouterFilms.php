        ...
        if (move_uploaded_file($file_2['tmp_name'], $path_2)) {
          ...
          if (($vod = fopen("data/vod.csv", "r")) !== FALSE) {
            ...
            while (($data = fgetcsv($vod,":")) !== FALSE) {
                $tmp = array();
                list($nomFilms, $annee_film, $realisateur_film) = explode(":", $data[0]);
                array_push($tmp, $nomFilms, $annee_film, str_replace('"', "", $realisateur_film));
          ...
          $newFilm = array();
          array_push($newFilm, $nomFilm, $annee, $realisateur);
